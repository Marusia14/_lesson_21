 ...
 */
function validateFileType(s) {
    return (/(\.jpg|\.jpeg|\.png)$/).test(s);
}
...
function checkYear(s) {
    return (/^(19\d\d|20\d\d|2100)$/).test(s);
}
...
function checkEmail(s) {
    return (/^[a-zA-z]+\W?[a-zA-z]+@[a-zA-z\.]+\.[a-z]{2,3}$/).test(s);
}
...
function checkDomainUrl(s) {
    return (/^(http:|https:)\/\/(?!:\/\/)([a-zA-Z0-9-_]+\.)*[a-zA-Z0-9][a-zA-Z0-9-_]+\.[a-zA-Z]{2,5}?$/).test(s);
}
...
function createLinksFromDomains(s) {
    let reg = new RegExp('(http:|https:)\\/\\/(?!:\\/\\/)([a-zA-Z0-9-_]+\\.)*[a-zA-Z0-9][a-zA-Z0-9-_]+\\.[a-zA-Z]{2,5}', 'gi');

    s.match(reg).forEach((item) => s = s.replace(item, `<a href="${item}" target="_blank">${item.replace(/^(http:|https:)\/\//gi, '')}</a>`));
